...
import Data.List (minimum, maximum, find, sort)
...
parse = sequence . fmap line
  where
    line = sequence . fmap readMaybe . words
...
-- Pre: List is sorted
pair :: Integral a => [a] -> Maybe (a, a)
pair [] = Nothing
pair (x : xs) 
  = case find (\y -> y `rem` x == 0) xs of
      Nothing  -> pair xs
      (Just y) -> Just (y, x)

q2 :: [[Int]] -> Maybe Int
q2 = fmap sum . sequence . fmap (f . sort)
  where
    f = fmap (uncurry div) . pair
  
main = do
  let ss = parse spreadsheet
  print $ fmap q1 ss
  print $ ss >>= q2
